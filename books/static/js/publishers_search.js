(function () {
  ...
  function initPublishersSearch(root = document) {
    const input = root.querySelector("#publishers-search-input");
    const resultsBox = root.querySelector("#publishers-search-results");
      ...
      resultsBox.innerHTML = items
        .map(
          (item) => `
            <button
              type="button"
              data-publisher-id="${escapeHtml(item.id)}"
              class="w-full text-left block px-3 py-1 hover:bg-gray-50">
              <div class="text-sm font-medium text-gray-900">${escapeHtml(item.publisher_name)}</div>
            </button>
          `
        )
        .join("");
    ...
    resultsBox.addEventListener("click", (e) => {
      const btn = e.target.closest("[data-publisher-id]");
      if (!btn) return;

      const id = btn.dataset.publisherId;

      ensureHiddenMulti("publishers", id);
    ...
    if (!globalOutsideClickBound) {
      ...
      document.addEventListener("click", (e) => {
        const isInside =
          e.target.closest("#publishers-search-results") ||
          e.target.closest("#publishers-search-input");
        if (!isInside) {
          const box = document.getElementById("publishers-search-results");
          if (box) {
  ...
  // Initial load
  document.addEventListener("DOMContentLoaded", () => initPublishersSearch(document));

  if (window.htmx && typeof window.htmx.onLoad === "function") {
    window.htmx.onLoad((content) => {
      initPublishersSearch(content);
    });
  }
})();
