(function () {
  ...
  function initAuthorsSearch(root = document) {
    const input = root.querySelector("#authors-search-input");
    const resultsBox = root.querySelector("#authors-search-results");
      ...
      resultsBox.innerHTML = items
        .map(
          (item) => `
            <button
              type="button"
              data-author-id="${escapeHtml(item.id)}"
    ...
    if (!globalOutsideClickBound) {
      ...
      document.addEventListener("click", (e) => {
        const isInside =
          e.target.closest("#authors-search-results") ||
          e.target.closest("#authors-search-input");
        if (!isInside) {
          const box = document.getElementById("authors-search-results");
          if (box) {
  ...
  document.addEventListener("DOMContentLoaded", () => initAuthorsSearch(document));

  if (window.htmx && typeof window.htmx.onLoad === "function") {
    window.htmx.onLoad((content) => {
      initAuthorsSearch(content);
    });
  }
})();
